...
SearchResult WeightedGraph::dijkstraVector(int source, int destination = INT_MAX) {
    SearchResult result;

    vector<int> distance(numVertices, INT_MAX); // Initializing all dist as 'infinite'
    ...
    for (int i = 1; i <= numVertices; i++) {
        ...
        for (int v = 1; v <= numVertices; v++)
            if (!visited[v-1] and (distance[v-1] < distance[i-1]))
                minVertex = v;

        if ((minVertex == destination) or // Min distance/path of destination vertex is found
            (distance[minVertex] == INT_MAX)) break; // No more reachable nodes
        
        visited[minVertex] = true; // Vertex is explored

        for (pair<int, int> neighbor : findNeighbors(minVertex)) {
            int neighborVertex = neighbor.first;
            int weight = neighbor.second;
            
            if (distance[minVertex - 1] + weight < distance[neighborVertex - 1]) {
                // Found a shorter path to neighbor
                distance[neighborVertex - 1] = distance[minVertex - 1] + weight;
...
and the spanning tree generated by the search starting from a source vertex up to a destination. If no destination is 
provided, the search is ran throughout the entire graph. */
SearchResult WeightedGraph::dijkstraHeap(int source, int destination = INT_MAX) {
    SearchResult result;
